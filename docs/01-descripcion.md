...
## Anexo — Cambios respecto de la versión anterior (`documentacion vieja/01-descripcion_VERSION_VIEJA.md`)

La versión vieja era un template incompleto con placeholders. Los cambios sustanciales al pasar a esta versión son:

| Cambio | Qué era antes | Qué es ahora | Por qué cambió |
|--------|--------------|--------------|----------------|
| **Sin control de stock** | El flujo step 4 decía "Seller App recibe la notificación y **actualiza el stock**" | Stock eliminado del dominio; toda publicación `active` tiene disponibilidad ilimitada (§1.1) | Decisión de alcance académico para simplificar la etapa. Sin stock no hay bloqueos de carrito ni `INSUFFICIENT_STOCK`, lo que simplifica el checkout multi-vendedor. |
| **Arquitectura multi-vendedor** | Flujo simple de 7 pasos, una sola orden | `order → N order_seller_groups → N sales_orders → N shipments → N settlements`; cada vendedor genera su propio ciclo completo | Refleja el modelo real de un marketplace: una compra puede contener productos de múltiples vendedores y cada uno opera de forma independiente. |
| **Comunicación inter-apps: REST puro** | Webhooks entre apps internas (`POST /api/webhooks/payment-confirmed`, `POST /api/webhooks/delivery-completed`) | Solo llamadas REST normales con `X-Service-Token`; el único webhook real es el de Mercado Pago → Payments | Los webhooks internos son REST con un nombre distinto. Usar REST directo con retry explícito da mayor control sobre errores, backoff y trazabilidad. |
| **Pagos vía Mercado Pago** | "Payments App procesa el cobro" sin especificar gateway | Integración explícita con Mercado Pago: `checkout_url`, webhook `payment.updated`, `transfer` para liquidaciones | Mercado Pago fue el gateway elegido; el diseño refleja su flujo real (webhook → resolver estado → notificar). |
| **Actores con roles y responsables** | Tabla de actores genérica sin roles ni responsables de app | Tabla con rol por actor (`publicMetadata.role`), responsable de cada app y qué datos propios tiene | Necesario para que los equipos sepan exactamente qué construir y cómo se autentica cada actor en el sistema. |
| **Flujos detallados con diagramas** | Ausente | §4 con 4 flujos Mermaid: compra multi-vendor, publicación, despacho/entrega, liquidación | Documentar los contratos cross-app visualmente evita malentendidos entre equipos durante la integración. |
