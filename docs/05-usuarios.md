...
## Anexo — Cambios respecto de `Docs Vieja/05-usuarios_doc_vieja.md`

| Cambio | Qué era antes | Qué es ahora | Por qué cambió |
|--------|--------------|--------------|----------------|
| **§1: arquitectura de identidad** | "Mapa de Clerks": tabla de 4 filas, cada app con su Clerk propio (`buyer.bicimarket`, `seller.bicimarket`, `shipping.bicimarket`, `payments.bicimarket`), audience (`aud`) distinto por app | "Clerk compartido": una única instancia, roles via `publicMetadata.role`, `publicMetadata.admin` como flag transversal | La arquitectura real usa un único Clerk. No existen cuatro instancias separadas ni audiences distintos por app. |
| **§3.2: alta de vendedor en primer login** | "Seller: crea `seller_profile` con `verification_status=pending_review`" — creación automática al primer login | "No crea `seller_profile` automáticamente. `requireAuth()` hace upsert en tabla auxiliar `User`. El vendedor debe llamar a `PUT /api/v1/seller-profile/me`; hasta entonces los endpoints devuelven `404 SELLER_PROFILE_NOT_FOUND`" | Crear el perfil vacío automáticamente requeriría campos obligatorios como `legal_name`, `tax_id`, `pickup_address` sin valores. Se optó por forzar que el vendedor los complete explícitamente. |
| **§4: claims del JWT** | Tabla separada por app con validaciones de `iss` y `aud` distintos por Clerk (ej: `iss=https://clerk.buyer.bicimarket.com`, `aud=bicimarket-buyer-api`) | Tabla unificada de claims comunes (`sub`, `email`, `email_verified`, `publicMetadata.role`, `publicMetadata.admin`) más validaciones adicionales por app en backend | Con Clerk único, todos los JWT tienen el mismo `iss` y no hay audiences separados. Los claims `publicMetadata.role` y `publicMetadata.admin` reemplazan la lógica de "pertenecer al Clerk correcto". |
| **§5.1 regla 1: rol funcional** | "El rol funcional es **implícito por el Clerk**. Si entrás con JWT de Clerk-Seller, sos seller" | "El rol funcional viene en `publicMetadata.role`. Sin ese claim, la app rechaza o trata al usuario como sin rol" | Con un único Clerk, no se puede inferir el rol por la instancia de Clerk. El rol debe ser explícito en el JWT. |
| **§5.2–5.5 diagramas de alta** | Participantes separados `Clerk-Buyer`, `Clerk-Seller`, `Clerk-Shipping`, `Clerk-Payments`; flujos sin paso de seteo de `publicMetadata.role` | Único participante `Clerk (compartido)` en todos los flujos; admin setea `publicMetadata.role` y `publicMetadata.admin` antes del primer login | Diagramas corregidos para reflejar la arquitectura real. Con Clerk único, el admin también debe asignar el rol antes de que el usuario acceda a la app correspondiente. |
| **§6: variables de entorno** | Keys distintas por app: `CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`, `CLERK_ISSUER`, `CLERK_AUDIENCE` con valores diferentes en cada app | Una sola `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` y `CLERK_SECRET_KEY` compartidas entre todas las apps | Con un único Clerk, las claves son las mismas. No existen `CLERK_ISSUER` ni `CLERK_AUDIENCE` separados por app. |
